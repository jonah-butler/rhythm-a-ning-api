...
func (u *UserHandler) RegisterUser(c *gin.Context) {
	...
	if results.UsernameTaken {
		respondErr(c, "registration failed - username unavailable", ErrUsernameTaken)
		return
	}
...
func (u *UserHandler) ReplayRegistration(c *gin.Context) {
	...
	userId, err := u.repo.GetUserIdByUsernameAndEmail(c, user.Username, user.Email)
	if err != nil {
	...
	inserted, err := u.repo.InsertUserRegistrationHash(c, userId, hash, expiration)
	if err != nil || !inserted {
	...
	responseSuccessNoContent(c, fmt.Sprintf("user registration replay successful: %d", userId))
}
...
func (u *UserHandler) AuthenticateUser(c *gin.Context) {
	...
	respondSuccessContent(c, fmt.Sprintf("authenticated user: %d", user.UserId), gin.H{"refresh": refreshToken, "jwt": jwt})
}

func (u *UserHandler) RefreshTokens(c *gin.Context) {
	token, err := c.Cookie(JWT)
	if err != nil {
		respondErr(c, "missing jwt cookie", ErrUnauthorized)
		return
	}
	...
	// JWT is expired — proceed with refresh
	...
	respondSuccessContent(c, "tokens refreshed", gin.H{"success": true})
}
